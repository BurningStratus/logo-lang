...
# ! priority
# Rewrite to use Token() class instead of tuples
##ZALIPUHA !!!#################################################################
def va_list_exec_t(fname, va_list=[]):
    # this wrapper handles the args. In a dictionary, one can set all args that
    # function expects, and this wrapper will parse out args and 
    # check types whether they are those function expects or not.

    # TODO: rethink and call procedures with args
    # TODO: add a flag to call with arbitrary amount of args
    def wrapper(self, *args, **kwargs):
        # check if arg_count exists
        try:
            arg_count - 1
        except:
            arg_count = 0
        # parse the needed amount of args

        #expected amount of args = len(va_list)
        #actual amount of args   = argc
        arg_list = []

        # if args are not parsed yet
        if arg_count == 0:
            for arg in va_list:
                if not self.parse_expression() and not self.parse_builtin_dictionary():
                    self.raise_error("Unknown statement")

            '''
            if len(va_list) > 0:
                if len(va_list) > arg_count:
                    self.raise_error("Not enough arguments supplied")
                elif argc > len(va_list):
                    self.raise_error("Too many arguments")
            '''
        for it in range(len(va_list)):
            arg_list.append(self.stack_pop())
            arg_count +=1
        ...
        try:
            ...
            self.raise_error(f"Procedure didn't like input {arg_list}")
    return wrapper
###############################################################################
...
class Logo(Logger, Debugger):
            ...
            while True: 
                ...
                if token.type == 'variable':
                    ...
                    if self.base_env.get_env(token.value):
                        list_.append(self.variable_dict[token.value])
                        continue
                    else:
                    ...
                    list_.append(self.stack_pop())

            self.stack_push(list_)

        elif token.type == 'variable':
            if token.value in self.variable_dict:
                self.stack_push(self.variable_dict[token.value])
            else:
                self.raise_error("Unknown variable name")

        '''
        elif token.value in self.subcommands:
            # exec subcommand
            self.subcommands[token.value]()
        '''
    ...
    def parse_builtin_dictionary(self, execute=True):
        ...
        # execute the function according to amt of args they take
        #self.builtins[token[0]](self)
